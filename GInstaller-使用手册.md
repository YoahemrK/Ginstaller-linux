...
- **`[Options]`可选参数：**
  ...
  --c,--setting  <!--显示已启动的饥荒联机版服务器-->
--au,--auto-update  <!--饥荒自动更新-->
  --ab,--auto-backup  <!--饥荒存档自动备份-->
  ```
...
系统时间（一般均为北京时间）**早上`6:30`检查**饥荒专服工具是否有新版本，如果有将更新游戏并重启所有正在运行的服务器

***注意！***如果你不希望服务器自动重启（比如你的服务器在`6:30`有人在玩），请不要开启自动更新。
...
备份在系统时间**`早晨9:00~次日凌晨1:00`时段每两小时执行一次，其余时段每四小时执行一次**（考虑到凌晨大多数人在睡觉，用不到备份）。**备份最多留存30份（即不间断备份时的3天）**，请注意保留的时间
...
2. ~~对于Debian系的Linux发行版（Debian 10和Ubuntu），对它们的支持推出之前，以下参数不能使用，其余参数可以直接运行~~
   ...
   v2.2.4版本更新，已加入对Debian和Ubuntu常用版本的支持

   **同时不受支持的Linux发行版将无法运行GInstaller**

3. “连续输入”指的是一次性输入多个参数。

4. TShock和tModLoader如果你实在找不到软件包，可以来我的QQ群里找。

5. 本项目现阶段不开源，但是你有任何代码方面的问题可以来找我，我可以提供帮助。
